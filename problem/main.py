...
if __name__ == "__main__":
    ...
    def model(x,y_,reuse=True):
        ...
        networkRGB = tl.layers.Conv2dLayer(networkRGB,act = tf.nn.relu,shape = (3, 3, 3, 64),strides = (1, 1, 1, 1), padding='VALID',W_init=tf.truncated_normal_initializer(stddev=5e-2), b_init = tf.constant_initializer(value=0.0), name ='cnn_layer1')
        #print (networkRGB)
        ...
        networkDepth = tl.layers.Conv2dLayer(networkDepth,act = tf.nn.relu,shape = (3, 3, 1, 64),strides = (1, 1, 1, 1), padding='VALID',W_init=tf.truncated_normal_initializer(stddev=5e-2), b_init = tf.constant_initializer(value=0.0), name ='cnn_layer1depth')
        ...
        networkRGB = tl.layers.Conv2dLayer(networkRGB,act = tf.nn.relu,shape = (3, 3, 64, 128),strides = (1, 1, 1, 1), padding='VALID',W_init=tf.truncated_normal_initializer(stddev=5e-2), b_init = tf.constant_initializer(value=0.1), name ='cnn_layer2')
        ...
        networkDepth = tl.layers.Conv2dLayer(networkDepth,act = tf.nn.relu,shape = (3, 3, 64, 128),strides = (1, 1, 1, 1), padding='VALID',W_init=tf.truncated_normal_initializer(stddev=5e-2), b_init = tf.constant_initializer(value=0.1), name ='cnn_layer2depth')
    ...
    train_op = tf.train.AdamOptimizer(learning_rate=0.00001, beta1=0.9, beta2=0.999,
    ...
    fit(session, network, train_op, cost, np.array(all_train_rgb_samples), np.array(all_train_depth_samples), np.array(all_train_labels), x, x_depth, y_,
                 acc=acc, batch_size=128, n_epoch=500, print_freq=1,
